...
export const actions = {
    async getNewsSlides({commit}) {
        const news = await this.$axios.$get('https://covid.info.az/api/slides');
    ...
    async getInterestingNews({commit}) {
        const interestingNews = await this.$axios.$get('https://covid.info.az/api/interesting?per_page=12')
        commit('SET_INTERESTING_NEWS', interestingNews.news.data)
    },
    async findNews({commit},id) {
        const news = await this.$axios.$get(`https://covid.info.az/api/news/${id}`);
        commit('GET_NEWS', news)
    },
    async getNewsData({commit}) {
        const news = await this.$axios.$get('https://covid.info.az/api/news');
        commit('SET_NEWS_DATA', news.news.data)
    },
    async getCats({commit}) {
        const cats = await this.$axios.$get('https://covid.info.az/api/cats');
        commit('SET_CATS_DATA', cats.cats)
    },
    async getVideoNews({commit}) {
        const cats = await this.$axios.$get('https://covid.info.az/api/videos?per_page=12');
        commit('SET_VIDEO_DATA', cats.news.data)
    },
    async getBanners({commit}) {
        const cats = await this.$axios.$get('https://covid.info.az/api/banners');
        commit('SET_BANNER_DATA', cats.banner)
    },
    async getPaginatedNews({commit}, data) {
        let res;
        if (data.id)
            res = await this.$axios.$get(`https://covid.info.az/api/cats/${data.id}?page=${data.curPage}&per_page=${data.perPage}`);
        else
            res = await this.$axios.$get(`https://covid.info.az/api/news?page=${data.curPage}&per_page=${data.perPage}`);
